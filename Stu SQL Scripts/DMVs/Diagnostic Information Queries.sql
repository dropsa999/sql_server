...
-- Hardware information from SQL Server 2008 and 2008 R2  (Query 6)
...
-- Get System Manufacturer and model number from  (Query 7) 
...
-- Get processor description from Windows Registry  (Query 8)
...
-- Get the current node name from your cluster nodes  (Query 9)
...
-- Get configuration values for instance  (Query 10)
...
-- SQL Server Registry information (SQL Server 2008 R2 SP1 or greater)  (Query 11)
...
-- Get information on location, time and size of any memory dumps from SQL Server  (Query 12)
...
-- File Names and Paths for TempDB and all user databases in instance  (Query 13) 
...
-- Volume info for all databases on the current instance (SQL Server 2008 R2 SP1 or greater)  (Query 14)
...
-- Recovery model, log reuse wait description, log file size, log usage size  (Query 15) 
...
-- Calculates average stalls per read, per write, and per total input/output for each database file  (Query 16) 
...
-- Isolate top waits for server instance since last restart or statistics clear  (Query 19)
WITH Waits AS
...
--  Get logins that are connected and how many sessions they have  (Query 21) 
...
-- Get Average Task Counts (run multiple times)  (Query 22)
...
-- Good basic information about OS memory amounts and state  (Query 24)
...
-- SQL Server Process Address space info  (Query 25) 
...
-- Memory Clerk Usage for instance  (Query 29)
...
-- Find single-use, ad-hoc queries that are bloating the plan cache  (Query 30)
...
-- Individual File Sizes and space available for current database  (Query 31)
...
-- I/O Statistics by file for the current database  (Query 32)
...
-- Get VLF count for transaction log for the current database,  (Query 33)
...
-- Top cached queries by Execution Count (SQL Server 2008)  (Query 34)
...
-- Top Cached SPs By Execution Count (SQL 2008)  (Query 35)
...
-- Lists the top statements by average input/output usage for the current database  (Query 42)
...
-- Possible Bad NC Indexes (writes > reads)  (Query 43)
...
-- Missing Indexes for current database by Index Advantage  (Query 44)
...
-- Breaks down buffers used by current database by object (table, index) in the buffer cache  (Query 46)
-- Note: This query could take some time on a busy instance
...
-- Get Table names, row counts, and compression status for clustered index or heap  (Query 47)
...
-- Get fragmentation info for all indexes above a certain size in the current database  (Query 49) 
...
--- Index Read/Write stats (all tables in current DB) ordered by Reads  (Query 50)
...
--- Index Read/Write stats (all tables in current DB) ordered by Writes  (Query 51)
SELECT OBJECT_NAME(s.[object_id]) AS [ObjectName], i.name AS [IndexName], i.index_id,
...
-- Look at recent Full backups for the current database (Query 52)
